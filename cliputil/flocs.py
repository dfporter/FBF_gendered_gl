...
class flocs:
    ...
    def __init__(self, transcript_id, chrom, txpt_left, txpt_right,
        ...
        while (seq[self.start_pos_in_seq:self.start_pos_in_seq+3] != 'ATG'):
            #print 'Flocs %s [%i:%i] = %s' % (
            #    self.transcript_id, self.start_pos_in_seq-1,
            #    self.start_pos_in_seq+3, seq[self.start_pos_in_seq-1:self.start_pos_in_seq+3])
            if cds_left < 1: break
            self.cds_left -= 1
            steps_backward += 1
...
""".format(
    ...
    def normalize_features(self):
        ...
        for (norm_set, motif_set) in [(self.norm_neg1, self.neg1_locs),
                                      (self.norm_neg2, self.neg2_locs),
                                      (self.norm_neg3, self.neg3_locs)]:
    ...
    def get_norm_pos(self, pos, test=''):
        ...
        if float(self.start_pos_in_seq) <= pos <= float(self.stop_pos_in_seq):
            ...
            norml = int(
                norm_left_len + int(float(pos_in_cds) * norm_cds_len/float(
                    self.stop_pos_in_seq - self.start_pos_in_seq))
                )
