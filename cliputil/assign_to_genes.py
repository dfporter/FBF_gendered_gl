...
"""config.ini file, only needed for the path to a gtf file. Default:./auto.ini""",
                        default='auto.ini')
...
def run(bed_folder=None, gtf_filename='', use_this_lib=None):
    ...
    for bed_filename in glob.glob(bed_folder + '/*.bed'):
        ...
        if not re.search('control_n2.bed', bed_filename): continue
...
if __name__ == '__main__':
    ...
    run(bed_folder=args.bed, gtf_filename=gtf_filename)
